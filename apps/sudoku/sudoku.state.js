(function() {
	...
	function updateCell(cell, flagNum, value) {
		//{index, flag, value}
		var flag = cell.flags[flagNum];//get the existing flag setting
		...
		var c2 = JSON.parse(JSON.stringify(cell));//deep clone
	...
	function doUpdateCells(cells, payload) {
		cells = [].concat(cells);
		var cell = cells[payload.index];
		var c2 = updateCell(cell, payload.flag, payload.value);
		if(c2!==cell) {
	...
	function selectedCellR(state, action) {
		var val = state.selectedCell; //may be undefined
		switch(action.type) {
			case "SELECT_CELL":
